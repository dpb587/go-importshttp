...
// as the VCS will force this factory to be used (or cause a failure if the match fails).
//
...
func (rf RepositoryFactory) matchURL(parsed *url.URL) bool {
	serverSplit := strings.SplitN(rf.Server, "://", 2)

	if len(parsed.Scheme) == 0 && serverSplit[1] == parsed.Host {
		return true
	} else if serverSplit[0] == parsed.Scheme && serverSplit[1] == parsed.Host {
		return true
	}
