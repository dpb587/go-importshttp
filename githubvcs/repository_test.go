...
func Test_Repository_Implicit(t *testing.T) {
	...
	if _e, _a := importshttp.GitVCS, subject.RepositoryVCS(); _e != _a {
		...
		t.Fatalf("expected `%v` but got: %v", _e, _a)
	} else if _e, _a := "https://github.com/test-owner-1/test-repository-1", subject.SourceURL(); _e != _a {
		t.Fatalf("expected `%v` but got: %v", _e, _a)
	} else if _e, _a := "https://github.com/test-owner-1/test-repository-1/tree/main{/dir}", subject.SourceDirTemplateURL(); _e != _a {
		t.Fatalf("expected `%v` but got: %v", _e, _a)
	} else if _e, _a := "https://github.com/test-owner-1/test-repository-1/blob/main{/dir}/{file}#L{line}", subject.SourceFileTemplateURL(); _e != _a {
		t.Fatalf("expected `%v` but got: %v", _e, _a)
	}
}
